...
static unsigned long dsi_get_txbyteclkhs(struct platform_device *dsidev)
{
	...
	/* CLKIN4DDR / 16 */
	return dsi->current_cinfo.params.clkout1 / 16;
}
...
{
	unsigned long dsi_fclk = cinfo->params.clkout_hsdiv[1];
	unsigned lp_clk_div;
...
static void dsi_pll_calc_dsi_fck(struct dsi_clock_info *cinfo)
{
	...
	cinfo->params.regm_hsdiv[HSDIV_DSI] =
		DIV_ROUND_UP(cinfo->params.clkout1, max_dsi_fck);

	cinfo->params.clkout_hsdiv[HSDIV_DSI] =
		cinfo->params.clkout1 / cinfo->params.regm_hsdiv[1];
}
...
static int dsi_pll_set_clock_div_new(struct platform_device *dsidev,
		struct dsi_clock_info *cinfo)
{
	struct dsi_data *dsi = dsi_get_dsidrv_data(dsidev);
	int r;
	...
	DSSDBG("CLKIN4DDR = 2 * %d / %d * %lu = %lu\n",
		...
		cinfo->params.regn,
		cinfo->params.clkin,
		cinfo->params.clkout1);

	DSSDBG("Data rate on 1 DSI lane %ld Mbps\n",
		cinfo->params.clkout1 / 1000 / 1000 / 2);

	DSSDBG("Clock lane freq %ld Hz\n", cinfo->params.clkout1 / 4);
	...
	/*
	 * temporary HACK: the DISPC hsdiv clock is used in dispc.c, update the
	 * older dsi_cinfo param so that dispc knows the updated value, this
	 * will be removed later
	 */
	dsi->current_cinfo.dsi_pll_hsdiv_dispc_clk =
		dsi->current_cinfo.params.clkout_hsdiv[HSDIV_DISPC];
		...
		struct seq_file *s)
{
	...
	seq_printf(s,	"Fint\t\t%-16luregn %u\n", cinfo->params.fint, cinfo->params.regn);

	seq_printf(s,	"CLKIN4DDR\t%-16luregm %u\n",
			cinfo->params.clkout1, cinfo->params.regm);
...
static inline unsigned ns2ddr(struct platform_device *dsidev, unsigned ns)
{
	...
	unsigned long ddr_clk = dsi->current_cinfo.params.clkout1 / 4;
...
static int dsi_configure_dsi_clocks(struct platform_device *dsidev)
{
	...
	r = dsi_pll_set_clock_div_new(dsidev, &cinfo);
	if (r) {
		...
		unsigned long pll, void *data)
{
	...
	ctx->dsi_cinfo.params.clkout1 = pll;
...
static bool dsi_vm_calc_blanking(struct dsi_clk_calc_ctx *ctx)
{
	...
	int ndl = dsi->num_lanes_used - 1;
	unsigned long hsclk = ctx->dsi_cinfo.params.clkout1 / 4;
...
static int omap_dsihw_probe(struct platform_device *dsidev)
{
	...
	/* use PLL code in dsi.c for now */
	dsi->pll_base = dsi->pll->base;
	dsi->sys_clk = dsi->pll->clkin;
