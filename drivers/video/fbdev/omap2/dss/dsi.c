...
#define DSI_FLD_GET(fld, start, end)\
	...
	/* -PLL (%d) */
	pr_debug("DSI resets: CIO (%d) PHY (%x%x%x, %d, %d, %d)\n",
...
unsigned long dsi_get_pll_hsdiv_dispc_rate(struct platform_device *dsidev)
{
	...
	return dsi->current_cinfo.pll_params.clkout_hsdiv[HSDIV_DISPC];
}
...
static unsigned long dsi_get_pll_hsdiv_dsi_rate(struct platform_device *dsidev)
{
	...
	return dsi->current_cinfo.pll_params.clkout_hsdiv[HSDIV_DSI];
}
...
static unsigned long dsi_get_txbyteclkhs(struct platform_device *dsidev)
{
	...
	/* CLKIN4DDR / 16 */
	return dsi->current_cinfo.pll_params.clkout1 / 16;
}
...
{
	unsigned long dsi_fclk = cinfo->pll_params.clkout_hsdiv[HSDIV_DSI];
	unsigned lp_clk_div;
...
static void dsi_pll_calc_dsi_fck(struct dsi_clock_info *cinfo)
{
	...
	cinfo->pll_params.regm_hsdiv[HSDIV_DSI] =
		DIV_ROUND_UP(cinfo->pll_params.clkout1, max_dsi_fck);

	cinfo->pll_params.clkout_hsdiv[HSDIV_DSI] =
		cinfo->pll_params.clkout1 /
			cinfo->pll_params.regm_hsdiv[HSDIV_DSI];
}
		...
		struct dsi_clock_info *cinfo)
{
	...
	DSSDBG("DSI Fint %ld\n", cinfo->pll_params.fint);

	DSSDBG("clkin rate %ld\n", cinfo->pll_params.clkin);

	/* DSIPHY == CLKIN4DDR */
	DSSDBG("CLKIN4DDR = 2 * %d / %d * %lu = %lu\n",
			cinfo->pll_params.regm,
			cinfo->pll_params.regn,
			cinfo->pll_params.clkin,
			cinfo->pll_params.clkout1);

	DSSDBG("Data rate on 1 DSI lane %ld Mbps\n",
			cinfo->pll_params.clkout1 / 1000 / 1000 / 2);

	DSSDBG("Clock lane freq %ld Hz\n", cinfo->pll_params.clkout1 / 4);

	DSSDBG("regm_dispc = %d, %s (%s) = %lu\n",
		cinfo->pll_params.regm_hsdiv[HSDIV_DISPC],
		dss_get_generic_clk_source_name(OMAP_DSS_CLK_SRC_DSI_PLL_HSDIV_DISPC),
		dss_feat_get_clk_source_name(OMAP_DSS_CLK_SRC_DSI_PLL_HSDIV_DISPC),
		cinfo->pll_params.clkout_hsdiv[HSDIV_DISPC]);
	...
	cinfo->pll_params.hsdiv_enabled[HSDIV_DISPC] = true;
	cinfo->pll_params.hsdiv_enabled[HSDIV_DSI] = true;
...
err0:
	...
	pll_enable_clock(dsi->pll, false);
	return r;
}
		...
		struct seq_file *s)
{
	...
	seq_printf(s,	"CLKIN4DDR\t%-16luregm %u\n",
			cinfo->pll_params.clkout1, cinfo->pll_params.regm);
...
static inline unsigned ns2ddr(struct platform_device *dsidev, unsigned ns)
{
	...
	unsigned long ddr_clk = dsi->current_cinfo.pll_params.clkout1 / 4;
...
static void dsi_config_cmd_mode_interleaving(struct platform_device *dsidev)
{
	...
	int dsi_fclk_hsdiv =
		dsi->user_dsi_cinfo.pll_params.regm_hsdiv[HSDIV_DSI] + 1;
		...
		void *data)
{
	...
	ctx->dsi_cinfo.pll_params.regm_hsdiv[HSDIV_DISPC] = regm_dispc;
	ctx->dsi_cinfo.pll_params.clkout_hsdiv[HSDIV_DISPC] = dispc;
...
static bool dsi_vm_calc_blanking(struct dsi_clk_calc_ctx *ctx)
{
	...
	int ndl = dsi->num_lanes_used - 1;
	unsigned long hsclk = ctx->dsi_cinfo.pll_params.clkout1 / 4;
