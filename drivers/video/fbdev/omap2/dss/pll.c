...
int pll_calc_and_check_clock_rates(struct pll_data *pll, struct pll_params *params)
{
	int i;
	...
	if (params->fint > feats->fint_max || params->fint < feats->fint_min)
		return -EINVAL;

	params->clkout1 = 2 * params->regm * params->fint;

	if (params->clkout1 > feats->clkout_max)
		return -EINVAL;

	for (i = 0; i < 4; i++) {
		if (params->regm_hsdiv[i] > 0)
			params->clkout_hsdiv[i] =
				params->clkout1 / params->regm_hsdiv[i];
		else
		...
		unsigned long clkout_max, pll_calc_func func, void *data)
{
	...
	for (regn = regn_start; regn <= regn_stop; ++regn) {
		...
		regm_start = max(DIV_ROUND_UP(DIV_ROUND_UP(clkout_min, fint), 2),
				1ul);
...
int pll_set_clock_div(struct pll_data *pll, struct pll_params *params)
{
	...
	BUG_ON(params->fint < feats->fint_min || params->fint > feats->fint_max);
	...
	} else if (feats->selffreqdco) {
		/* update ranges according to PLL type */
		f = params->clkout1 < feats->dco_range2_min ?
			0x2 : 0x4;
	...
	if (pll->type == DSS_PLL_TYPE_HDMI) {
		...
		l = FLD_MOD(r, params->regm2, 24, 18);
		l = FLD_MOD(r, params->regmf, 17, 0);
		pll_write_reg(pll->base, PLL_CONFIGURATION4, r);
	}
	...
	l = FLD_MOD(l, params->hsdiv_enabled[0], 16, 16);	/* HSDIV1_CLOCK_EN */
	l = FLD_MOD(l, 0, 17, 17);	/* DSS_CLOCK_PWDN */
	l = FLD_MOD(l, params->hsdiv_enabled[1], 18, 18);	/* HSDIV2_CLOCK_EN */
	l = FLD_MOD(l, 0, 19, 19);	/* PROTO_CLOCK_PWDN */
	l = FLD_MOD(l, 0, 20, 20);	/* HSDIVBYPASS */
	l = FLD_MOD(l, params->hsdiv_enabled[2], 23, 23);	/* HSDIV3_CLOCK_EN */
	l = FLD_MOD(l, params->hsdiv_enabled[3], 25, 25);	/* HSDIV4_CLOCK_EN */
	pll_write_reg(pll->base, PLL_CONFIGURATION2, l);
