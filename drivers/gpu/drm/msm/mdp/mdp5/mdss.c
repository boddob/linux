...
struct msm_mdss {
	...
	/* acts as GDSC for downstream kernels */
...
irqreturn_t mdss_irq(int irq, void *arg)
{
...
int mdss_irq_domain_init(struct msm_mdss *mdss)
{
...
void mdss_destroy(struct drm_device *dev)
{
...
int mdss_init(struct drm_device *dev)
{
	...
	if (of_device_is_compatible(dev->dev->of_node, "qcom,mdp4"))
		return 0;
	...
	mdss->vdd = devm_regulator_get(&pdev->dev, "vdd");
	if (IS_ERR(mdss->vdd)) {
