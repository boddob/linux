...
struct dsi_pll_14nm *pll_list[2];
				 ...
				 struct dsi_pll_output *pout)
{
	...
	/* REG_DSI_14nm_PHY_PLL_CLKBUFLR_EN updated at dsi phy */
 ...
 * Clock Callbacks
 */
static int dsi_pll_14nm_clk_set_rate(struct clk_hw *hw, unsigned long rate,
		unsigned long parent_rate)
{
	...
	if (1/*pll_14nm->uc == MSM_DSI_PHY_MASTER*/) {
		struct dsi_pll_14nm *pll_14nm_slave = pll_list[1];
...
static unsigned long dsi_pll_14nm_clk_recalc_rate(struct clk_hw *hw,
		unsigned long parent_rate)
{
	struct msm_dsi_pll *pll = hw_clk_to_pll(hw);
	struct dsi_pll_14nm *pll_14nm = to_pll_14nm(pll);
	...
	div_frac_start = (pll_read(base + REG_DSI_14nm_PHY_PLL_DIV_FRAC_START3) & 0x0f) << 16;
	div_frac_start |= (pll_read(base + REG_DSI_14nm_PHY_PLL_DIV_FRAC_START2) & 0x0ff) << 8;
	div_frac_start |= pll_read(base + REG_DSI_14nm_PHY_PLL_DIV_FRAC_START1) & 0x0ff;
...
static const struct clk_ops clk_ops_dsi_pll_14nm_vco = {
	...
	.set_rate = dsi_pll_14nm_clk_set_rate,
	.recalc_rate = dsi_pll_14nm_clk_recalc_rate,
...
static int dsi_pll_14nm_restore_state(struct msm_dsi_pll *pll)
{
	...
	ret = dsi_pll_14nm_clk_set_rate(&pll->clk_hw,
...
static int pll_14nm_register(struct dsi_pll_14nm *pll_14nm)
{
	...
	ret = of_clk_add_provider(dev->of_node,
			of_clk_src_onecell_get, &pll_14nm->clk_data);
	if (ret) {
