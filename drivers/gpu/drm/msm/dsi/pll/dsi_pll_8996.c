/*
 * Copyright (c) 2012-2015, The Linux Foundation. All rights reserved.
 *
 * This program is free software; you can redistribute it and/or modify
 * it under the terms of the GNU General Public License version 2 and
 * only version 2 as published by the Free Software Foundation.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 */

#include <linux/clk.h>
#include <linux/clk-provider.h>

#include "dsi_pll.h"
#include "dsi.xml.h"
#include "dsi_pll_8996.h"

/*
 * DSI PLL 8996 - clock diagram (eg: DSI0):
 *
 *                 +----+     +----+
 *  dsi0vco_clk ---| n1 |--o--| /8 |-- dsi0pllbyte
 *                 +----+  |  +----+
 *                         |
 *                         |   +----+
 *                         o---| /2 |---|\
 *                         |   +----+   | \   +----+
 *                         |            |  |--| n2 |--dsi0pll
 *                         o------------| /   +----+
 *                                      |/
 */

#define POLL_MAX_READS			15
#define POLL_TIMEOUT_US			1000

#define NUM_PROVIDED_CLKS		2

#define VCO_REF_CLK_RATE		19200000
#define VCO_MIN_RATE			1300000000UL
#define VCO_MAX_RATE			2600000000UL

#define DSI_BYTE_PLL_CLK		0
#define DSI_PIXEL_PLL_CLK		1

#define DSI_PLL_DEFAULT_VCO_POSTDIV	1

#define CEIL(x, y)		(((x) + ((y)-1)) / (y))

struct pll_8996_cached_state {
	unsigned long vco_rate;
	u8 n2postdiv;
	u8 n1postdiv;
};

struct dsi_pll_8996 {
	struct msm_dsi_pll base;

	int id;
	struct platform_device *pdev;

	void __iomem *phy_cmn_mmio;
	void __iomem *mmio;

	int vco_delay;

	struct dsi_pll_input in;
	struct dsi_pll_output out;

	bool ssc_en;		/* share pll with master */
	bool ssc_center;	/* default is down spread */
	u32 ssc_freq;
	u32 ssc_ppm;

	/* maybe remove? */
	s64 vco_current_rate;
	s64 vco_locking_rate;
	s64 vco_ref_clk_rate;

	/* private clocks: */
	struct clk *clks[NUM_DSI_CLOCKS_MAX];
	u32 num_clks;

	/* clock-provider: */
	struct clk *provided_clks[NUM_PROVIDED_CLKS];
	struct clk_onecell_data clk_data;

	struct pll_8996_cached_state cached_state;

	enum msm_dsi_phy_usecase uc;
};

struct dsi_pll_8996 *pll_list[2];

#define to_pll_8996(x)	container_of(x, struct dsi_pll_8996, base)

static bool pll_8996_poll_for_ready(struct dsi_pll_8996 *pll_8996,
				u32 nb_tries, u32 timeout_us)
{
	bool pll_locked = false;
	void __iomem *base = pll_8996->mmio;
	u32 tries, val;

	tries = nb_tries;
	while (tries--) {
		val = pll_read(base + DSIPHY_PLL_RESET_SM_READY_STATUS);
		pll_locked = !!(val & BIT(5));

		if (pll_locked)
			break;

		udelay(timeout_us);
	}

	if (!pll_locked) {
		tries = nb_tries;
		while (tries--) {
			val = pll_read(base + DSIPHY_PLL_RESET_SM_READY_STATUS);
			pll_locked = !!(val & BIT(0));

			if (pll_locked)
				break;

			udelay(timeout_us);
		}
	}

	DBG("DSI PLL is %slocked", pll_locked ? "" : "*not* ");

	return pll_locked;
}

static void dsi_pll_8996_input_init(struct dsi_pll_8996 *pll)
{
	pll->in.fref = VCO_REF_CLK_RATE;	/* 19.2 Mhz*/
	pll->in.fdata = 0;		/* bit clock rate */
	pll->in.dsiclk_sel = 1;		/* 1, reg: 0x0014 */
	pll->in.ssc_en = pll->ssc_en;		/* 1, reg: 0x0094, bit 0 */
	pll->in.ldo_en = 0;		/* 0,  reg: 0x004c, bit 0 */

	/* fixed  input */
	pll->in.refclk_dbler_en = 0;	/* 0, reg: 0x00c0, bit 1 */
	pll->in.vco_measure_time = 5;	/* 5, unknown */
	pll->in.kvco_measure_time = 5;	/* 5, unknown */
	pll->in.bandgap_timer = 4;	/* 4, reg: 0x0030, bit 3 - 5 */
	pll->in.pll_wakeup_timer = 5;	/* 5, reg: 0x003c, bit 0 - 2 */
	pll->in.plllock_cnt = 1;	/* 1, reg: 0x0088, bit 1 - 2 */
	pll->in.plllock_rng = 0;	/* 0, reg: 0x0088, bit 3 - 4 */
	pll->in.ssc_center = pll->ssc_center;/* 0, reg: 0x0094, bit 1 */
	pll->in.ssc_adj_period = 37;	/* 37, reg: 0x0098, bit 0 - 9 */
	pll->in.ssc_spread = pll->ssc_ppm / 1000;
	pll->in.ssc_freq = pll->ssc_freq;

	pll->in.pll_ie_trim = 4;	/* 4, reg: 0x0000 */
	pll->in.pll_ip_trim = 4;	/* 4, reg: 0x0004 */
	pll->in.pll_cpcset_cur = 1;	/* 1, reg: 0x00f0, bit 0 - 2 */
	pll->in.pll_cpmset_cur = 1;	/* 1, reg: 0x00f0, bit 3 - 5 */
	pll->in.pll_icpmset = 4;	/* 4, reg: 0x00fc, bit 3 - 5 */
	pll->in.pll_icpcset = 4;	/* 4, reg: 0x00fc, bit 0 - 2 */
	pll->in.pll_icpmset_p = 0;	/* 0, reg: 0x00f4, bit 0 - 2 */
	pll->in.pll_icpmset_m = 0;	/* 0, reg: 0x00f4, bit 3 - 5 */
	pll->in.pll_icpcset_p = 0;	/* 0, reg: 0x00f8, bit 0 - 2 */
	pll->in.pll_icpcset_m = 0;	/* 0, reg: 0x00f8, bit 3 - 5 */
	pll->in.pll_lpf_res1 = 3;	/* 3, reg: 0x0104, bit 0 - 3 */
	pll->in.pll_lpf_cap1 = 11;	/* 11, reg: 0x0100, bit 0 - 3 */
	pll->in.pll_lpf_cap2 = 1;	/* 1, reg: 0x0100, bit 4 - 7 */
	pll->in.pll_iptat_trim = 7;
	pll->in.pll_c3ctrl = 2;		/* 2 */
	pll->in.pll_r3ctrl = 1;		/* 1 */
}

static void pll_8996_ssc_calc(struct dsi_pll_8996 *pll)
{
	u32 period, ssc_period;
	u32 ref, rem;
	s64 step_size;

	DBG("vco=%lld ref=%lld", pll->vco_current_rate, pll->vco_ref_clk_rate);

	ssc_period = pll->in.ssc_freq / 500;
	period = (unsigned long)pll->vco_ref_clk_rate / 1000;
	ssc_period  = CEIL(period, ssc_period);
	ssc_period -= 1;
	pll->out.ssc_period = ssc_period;

	DBG("ssc freq=%d spread=%d period=%d", pll->in.ssc_freq, pll->in.ssc_spread, pll->out.ssc_period);

	step_size = (u32)pll->vco_current_rate;
	ref = pll->vco_ref_clk_rate;
	ref /= 1000;
	step_size = div_s64(step_size, ref);
	step_size <<= 20;
	step_size = div_s64(step_size, 1000);
	step_size *= pll->in.ssc_spread;
	step_size = div_s64(step_size, 1000);
	step_size *= (pll->in.ssc_adj_period + 1);

	rem = 0;
	step_size = div_s64_rem(step_size, ssc_period + 1, &rem);
	if (rem)
		step_size++;

	DBG("step_size=%lld", step_size);

	step_size &= 0x0ffff;	/* take lower 16 bits */

	pll->out.ssc_step_size = step_size;
}

static void pll_8996_dec_frac_calc(struct dsi_pll_8996 *pll)
{
	struct dsi_pll_input *pin = &pll->in;
	struct dsi_pll_output *pout = &pll->out;
	s64 multiplier = BIT(20);
	s64 dec_start_multiple, dec_start, pll_comp_val;
	s32 duration, div_frac_start;
	s64 vco_clk_rate = pll->vco_current_rate;
	s64 fref = pll->vco_ref_clk_rate;

	DBG("vco_clk_rate=%lld ref_clk_rate=%lld", vco_clk_rate, fref);

	dec_start_multiple = div_s64(vco_clk_rate * multiplier, fref);
	div_s64_rem(dec_start_multiple, multiplier, &div_frac_start);

	dec_start = div_s64(dec_start_multiple, multiplier);

	pout->dec_start = (u32)dec_start;
	pout->div_frac_start = div_frac_start;

	if (pin->plllock_cnt == 0)
		duration = 1024;
	else if (pin->plllock_cnt == 1)
		duration = 256;
	else if (pin->plllock_cnt == 2)
		duration = 128;
	else
		duration = 32;

	pll_comp_val =  duration * dec_start_multiple;
	pll_comp_val =  div_s64(pll_comp_val, multiplier);
	do_div(pll_comp_val, 10);

	pout->plllock_cmp = (u32)pll_comp_val;

	pout->pll_txclk_en = 1;
	/* DSI PLL revision 2 */
	pout->cmn_ldo_cntrl = 0x3c;
}

static u32 pll_8996_kvco_slop(u32 vrate)
{
	u32 slop = 0;

	if (vrate > VCO_MIN_RATE && vrate <= 1800000000UL)
		slop =  600;
	else if (vrate > 1800000000UL && vrate < 2300000000UL)
		slop = 400;
	else if (vrate > 2300000000UL && vrate < VCO_MAX_RATE)
		slop = 280;

	return slop;
}

static void pll_8996_calc_vco_count(struct dsi_pll_8996 *pll,
			 s64 vco_clk_rate, s64 fref)
{
	struct dsi_pll_input *pin = &pll->in;
	struct dsi_pll_output *pout = &pll->out;
	s64 data;
	u32 cnt;

	data = fref * pin->vco_measure_time;
	do_div(data, 1000000);
	data &= 0x03ff;	/* 10 bits */
	data -= 2;
	pout->pll_vco_div_ref = data;

	data = (unsigned long)vco_clk_rate / 1000000;	/* unit is Mhz */
	data *= pin->vco_measure_time;
	do_div(data, 10);
	pout->pll_vco_count = data; /* reg: 0x0474, 0x0478 */

	data = fref * pin->kvco_measure_time;
	do_div(data, 1000000);
	data &= 0x03ff;	/* 10 bits */
	data -= 1;
	pout->pll_kvco_div_ref = data;

	cnt = pll_8996_kvco_slop(vco_clk_rate);
	cnt *= 2;
	do_div(cnt, 100);
	cnt *= pin->kvco_measure_time;
	pout->pll_kvco_count = cnt;

	pout->pll_misc1 = 16;
	pout->pll_resetsm_cntrl = 48;
	pout->pll_resetsm_cntrl2 = pin->bandgap_timer << 3;
	pout->pll_resetsm_cntrl5 = pin->pll_wakeup_timer;
	pout->pll_kvco_code = 0;
}

static void pll_db_commit_ssc(struct dsi_pll_8996 *pll)
{
	void __iomem *base = pll->mmio;
	struct dsi_pll_input *pin = &pll->in;
	struct dsi_pll_output *pout = &pll->out;
	u8 data;

	data = pin->ssc_adj_period;
	data &= 0x0ff;
	pll_write(base + DSIPHY_PLL_SSC_ADJ_PER1, data);
	data = (pin->ssc_adj_period >> 8);
	data &= 0x03;
	pll_write(base + DSIPHY_PLL_SSC_ADJ_PER2, data);

	data = pout->ssc_period;
	data &= 0x0ff;
	pll_write(base + DSIPHY_PLL_SSC_PER1, data);
	data = (pout->ssc_period >> 8);
	data &= 0x0ff;
	pll_write(base + DSIPHY_PLL_SSC_PER2, data);

	data = pout->ssc_step_size;
	data &= 0x0ff;
	pll_write(base + DSIPHY_PLL_SSC_STEP_SIZE1, data);
	data = (pout->ssc_step_size >> 8);
	data &= 0x0ff;
	pll_write(base + DSIPHY_PLL_SSC_STEP_SIZE2, data);

	data = (pin->ssc_center & 0x01);
	data <<= 1;
	data |= 0x01; /* enable */
	pll_write(base + DSIPHY_PLL_SSC_EN_CENTER, data);

	wmb();	/* make sure register committed */
}

static void pll_db_commit_common(struct dsi_pll_8996 *pll,
				 struct dsi_pll_input *pin,
				 struct dsi_pll_output *pout)
{
	void __iomem *base = pll->mmio;
	u8 data;

	/* confgiure the non frequency dependent pll registers */
	data = 0;
	pll_write(base + DSIPHY_PLL_SYSCLK_EN_RESET, data);

	/* DSIPHY_PLL_CLKBUFLR_EN updated at dsi phy */

	data = pout->pll_txclk_en;
	pll_write(base + DSIPHY_PLL_TXCLK_EN, data);

	data = pout->pll_resetsm_cntrl;
	pll_write(base + DSIPHY_PLL_RESETSM_CNTRL, data);
	data = pout->pll_resetsm_cntrl2;
	pll_write(base + DSIPHY_PLL_RESETSM_CNTRL2, data);
	data = pout->pll_resetsm_cntrl5;
	pll_write(base + DSIPHY_PLL_RESETSM_CNTRL5, data);

	data = pout->pll_vco_div_ref;
	data &= 0x0ff;
	pll_write(base + DSIPHY_PLL_VCO_DIV_REF1, data);
	data = (pout->pll_vco_div_ref >> 8);
	data &= 0x03;
	pll_write(base + DSIPHY_PLL_VCO_DIV_REF2, data);

	data = pout->pll_kvco_div_ref;
	data &= 0x0ff;
	pll_write(base + DSIPHY_PLL_KVCO_DIV_REF1, data);
	data = (pout->pll_kvco_div_ref >> 8);
	data &= 0x03;
	pll_write(base + DSIPHY_PLL_KVCO_DIV_REF2, data);

	data = pout->pll_misc1;
	pll_write(base + DSIPHY_PLL_PLL_MISC1, data);

	data = pin->pll_ie_trim;
	pll_write(base + DSIPHY_PLL_IE_TRIM, data);

	data = pin->pll_ip_trim;
	pll_write(base + DSIPHY_PLL_IP_TRIM, data);

	data = ((pin->pll_cpmset_cur << 3) | pin->pll_cpcset_cur);
	pll_write(base + DSIPHY_PLL_CP_SET_CUR, data);

	data = ((pin->pll_icpcset_p << 3) | pin->pll_icpcset_m);
	pll_write(base + DSIPHY_PLL_PLL_ICPCSET, data);

	data = ((pin->pll_icpmset_p << 3) | pin->pll_icpcset_m);
	pll_write(base + DSIPHY_PLL_PLL_ICPMSET, data);

	data = ((pin->pll_icpmset << 3) | pin->pll_icpcset);
	pll_write(base + DSIPHY_PLL_PLL_ICP_SET, data);

	data = ((pin->pll_lpf_cap2 << 4) | pin->pll_lpf_cap1);
	pll_write(base + DSIPHY_PLL_PLL_LPF1, data);

	data = pin->pll_iptat_trim;
	pll_write(base + DSIPHY_PLL_IPTAT_TRIM, data);

	data = (pin->pll_c3ctrl | (pin->pll_r3ctrl << 4));
	pll_write(base + DSIPHY_PLL_PLL_CRCTRL, data);
}

static void pll_8996_software_reset(struct dsi_pll_8996 *pll_8996)
{
	void __iomem *cmn_base = pll_8996->phy_cmn_mmio;

	/* de assert pll start and apply pll sw reset */
	/* stop pll */
	pll_write(cmn_base + DSIPHY_CMN_PLL_CNTRL, 0);

	/* pll sw reset */
	pll_write_udelay(cmn_base + DSIPHY_CMN_CTRL_1, 0x20, 10);
	wmb();	/* make sure register committed */

	pll_write(cmn_base + DSIPHY_CMN_CTRL_1, 0);
	wmb();	/* make sure register committed */
}

static void pll_db_commit_8996(struct dsi_pll_8996 *pll,
			       struct dsi_pll_input *pin,
			       struct dsi_pll_output *pout)
{
	void __iomem *base = pll->mmio;
	void __iomem *cmn_base = pll->phy_cmn_mmio;
	u8 data;

	DBG("PLL%d", pll->id);

	data = pout->cmn_ldo_cntrl;
	pll_write(cmn_base + DSIPHY_CMN_LDO_CNTRL, data);

	pll_db_commit_common(pll, pin, pout);

	pll_8996_software_reset(pll);

	data = pin->dsiclk_sel; /* set dsiclk_sel = 1  */
	pll_write(cmn_base + DSIPHY_CMN_CLK_CFG1, data);

	data = 0xff; /* data, clk, pll normal operation */
	pll_write(cmn_base + DSIPHY_CMN_CTRL_0, data);

	/* confgiure the frequency dependent pll registers */
	data = pout->dec_start;
	pll_write(base + DSIPHY_PLL_DEC_START, data);

	data = pout->div_frac_start;
	data &= 0x0ff;
	pll_write(base + DSIPHY_PLL_DIV_FRAC_START1, data);
	data = (pout->div_frac_start >> 8);
	data &= 0x0ff;
	pll_write(base + DSIPHY_PLL_DIV_FRAC_START2, data);
	data = (pout->div_frac_start >> 16);
	data &= 0x0f;
	pll_write(base + DSIPHY_PLL_DIV_FRAC_START3, data);

	data = pout->plllock_cmp;
	data &= 0x0ff;
	pll_write(base + DSIPHY_PLL_PLLLOCK_CMP1, data);
	data = (pout->plllock_cmp >> 8);
	data &= 0x0ff;
	pll_write(base + DSIPHY_PLL_PLLLOCK_CMP2, data);
	data = (pout->plllock_cmp >> 16);
	data &= 0x03;
	pll_write(base + DSIPHY_PLL_PLLLOCK_CMP3, data);

	data = ((pin->plllock_cnt << 1) | (pin->plllock_rng << 3));
	pll_write(base + DSIPHY_PLL_PLLLOCK_CMP_EN, data);

	data = pout->pll_vco_count;
	data &= 0x0ff;
	pll_write(base + DSIPHY_PLL_VCO_COUNT1, data);
	data = (pout->pll_vco_count >> 8);
	data &= 0x0ff;
	pll_write(base + DSIPHY_PLL_VCO_COUNT2, data);

	data = pout->pll_kvco_count;
	data &= 0x0ff;
	pll_write(base + DSIPHY_PLL_KVCO_COUNT1, data);
	data = (pout->pll_kvco_count >> 8);
	data &= 0x03;
	pll_write(base + DSIPHY_PLL_KVCO_COUNT2, data);

	data = (((pout->pll_postdiv - 1) << 4) | pin->pll_lpf_res1);
	pll_write(base + DSIPHY_PLL_PLL_LPF2_POSTDIV, data);

#if 0
	/* configured by fixed factor dividers */
	data = (pout->pll_n1div | (pout->pll_n2div << 4));
	pll_write(cmn_base + DSIPHY_CMN_CLK_CFG0, data);
#else
	DBG("CMN_CLK_CFG0 %x", pll_read(cmn_base + DSIPHY_CMN_CLK_CFG0));
#endif
	if (pll->ssc_en)
		pll_db_commit_ssc(pll);

	wmb();	/* make sure register committed */
}

/*
 * Clock Callbacks
 */
static int dsi_pll_8996_clk_set_rate(struct clk_hw *hw, unsigned long rate,
		unsigned long parent_rate)
{
	struct msm_dsi_pll *pll = hw_clk_to_pll(hw);
	struct dsi_pll_8996 *pll_8996 = to_pll_8996(pll);
	struct dsi_pll_input *pin = &pll_8996->in;
	struct dsi_pll_output *pout = &pll_8996->out;

	DBG("DSI PLL%d rate=%lu, parent's=%lu", pll_8996->id, rate, parent_rate);
	/* Write to DSIPHY_PLL_PLL_LPF2_POSTDIV and DSIPHY_CMN_CLK_CFG1 at some point here */

	pll_8996->vco_current_rate = rate;
	pll_8996->vco_ref_clk_rate = VCO_REF_CLK_RATE;

	dsi_pll_8996_input_init(pll_8996);
	/*
	 * tx_band = pll_postdiv
	 * 0: divided by 1 <== for now
	 * 1: divided by 2
	 * 2: divided by 4
	 * 3: divided by 8
	 */
	pll_8996->out.pll_postdiv = DSI_PLL_DEFAULT_VCO_POSTDIV;

	pll_8996_dec_frac_calc(pll_8996);

	if (pll_8996->ssc_en)
		pll_8996_ssc_calc(pll_8996);

	pll_8996_calc_vco_count(pll_8996, rate, VCO_REF_CLK_RATE);

	if (1/*pll_8996->uc == MSM_DSI_PHY_MASTER*/) {
		struct dsi_pll_8996 *pll_8996_slave = pll_list[1];

		/* HACK */
		pll_write(pll_8996_slave->phy_cmn_mmio + DSIPHY_CMN_CLK_CFG0, 0x32);
		pll_db_commit_8996(pll_8996_slave, pin, pout);
	}

	pll_db_commit_8996(pll_8996, pin, pout);

	return 0;
}

static unsigned long dsi_pll_8996_clk_recalc_rate(struct clk_hw *hw,
		unsigned long parent_rate)
{
	struct msm_dsi_pll *pll = hw_clk_to_pll(hw);
	struct dsi_pll_8996 *pll_8996 = to_pll_8996(pll);
	void __iomem *base = pll_8996->mmio;
	u64 vco_rate, multiplier = BIT(20);
	s32 div_frac_start;
	u32 dec_start;
	u64 ref_clk = VCO_REF_CLK_RATE;

	dec_start = pll_read(base + DSIPHY_PLL_DEC_START);
	dec_start &= 0x0ff;

	DBG("dec_start = %x", dec_start);

	div_frac_start = (pll_read(base + DSIPHY_PLL_DIV_FRAC_START3) & 0x0f) << 16;
	div_frac_start |= (pll_read(base + DSIPHY_PLL_DIV_FRAC_START2) & 0x0ff) << 8;
	div_frac_start |= pll_read(base + DSIPHY_PLL_DIV_FRAC_START1) & 0x0ff;

	DBG("div_frac_start = %x", div_frac_start);

	vco_rate = ref_clk * dec_start;
	vco_rate += ((ref_clk * div_frac_start) / multiplier);

	DBG("returning vco rate = %lu", (unsigned long)vco_rate);

	return (unsigned long)vco_rate;
}

static const struct clk_ops clk_ops_dsi_pll_8996_vco = {
	.round_rate = msm_dsi_pll_helper_clk_round_rate,
	.set_rate = dsi_pll_8996_clk_set_rate,
	.recalc_rate = dsi_pll_8996_clk_recalc_rate,
	.prepare = msm_dsi_pll_helper_clk_prepare,
	.unprepare = msm_dsi_pll_helper_clk_unprepare,
	.is_enabled = msm_dsi_pll_helper_clk_is_enabled,
};

/*
 * PLL Callbacks
 */

static int dsi_pll_8996_enable_seq(struct msm_dsi_pll *pll)
{
	struct dsi_pll_8996 *pll_8996 = to_pll_8996(pll);
	void __iomem *base = pll_8996->mmio;
	void __iomem *cmn_base = pll_8996->phy_cmn_mmio;
	bool locked;

	DBG("");

	pll_write(base + DSIPHY_PLL_VREF_CFG1, 0x10);
	pll_write(cmn_base + DSIPHY_CMN_PLL_CNTRL, 1);

	locked = pll_8996_poll_for_ready(pll_8996, POLL_MAX_READS, POLL_TIMEOUT_US);

	if (unlikely(!locked))
		dev_err(&pll_8996->pdev->dev, "DSI PLL lock failed\n");
	else
		DBG("DSI PLL lock success");

	return locked ? 0 : -EINVAL;
}

static void dsi_pll_8996_disable_seq(struct msm_dsi_pll *pll)
{
	struct dsi_pll_8996 *pll_8996 = to_pll_8996(pll);
	void __iomem *cmn_base = pll_8996->phy_cmn_mmio;

	DBG("");

	pll_write(cmn_base + DSIPHY_CMN_PLL_CNTRL, 0);
}

static void dsi_pll_8996_save_state(struct msm_dsi_pll *pll)
{
	struct dsi_pll_8996 *pll_8996 = to_pll_8996(pll);
	struct pll_8996_cached_state *cached_state = &pll_8996->cached_state;
	void __iomem *cmn_base = pll_8996->phy_cmn_mmio;
	u32 data;

	data = pll_read(cmn_base + DSIPHY_CMN_CLK_CFG0);

	cached_state->n1postdiv = data & 0xf;
	cached_state->n2postdiv = (data >> 4) & 0xf;

	DBG("save state %x %x", cached_state->n1postdiv, cached_state->n2postdiv);

	cached_state->vco_rate = clk_hw_get_rate(&pll->clk_hw);
}

static int dsi_pll_8996_restore_state(struct msm_dsi_pll *pll)
{
	struct dsi_pll_8996 *pll_8996 = to_pll_8996(pll);
	struct pll_8996_cached_state *cached_state = &pll_8996->cached_state;
	void __iomem *cmn_base = pll_8996->phy_cmn_mmio;
	u32 data;
	int ret;

	ret = dsi_pll_8996_clk_set_rate(&pll->clk_hw,
					cached_state->vco_rate, 0);
	if (ret) {
		dev_err(&pll_8996->pdev->dev,
			"restore vco rate failed. ret=%d\n", ret);
		return ret;
	}

	data = cached_state->n1postdiv | ( cached_state->n2postdiv << 4);
	DBG("restore state %x %x", cached_state->n1postdiv, cached_state->n2postdiv);
	pll_write(cmn_base + DSIPHY_CMN_CLK_CFG0, 0x32 /*data */);

	return 0;
}

static int dsi_pll_8996_set_usecase(struct msm_dsi_pll *pll,
				    enum msm_dsi_phy_usecase uc)
{
	struct dsi_pll_8996 *pll_8996 = to_pll_8996(pll);
	void __iomem *base = pll_8996->mmio;
	u32 clkbuflr_en, bandgap = 0;

	switch (uc) {
	case MSM_DSI_PHY_STANDALONE:
		clkbuflr_en = 0x1;
		break;
	case MSM_DSI_PHY_MASTER:
		clkbuflr_en = 0x3;
		break;
	case MSM_DSI_PHY_SLAVE:
		clkbuflr_en = 0x0;
		bandgap = 0x3;
		break;
	default:
		return -EINVAL;
	}

	pll_write(base + DSIPHY_PLL_CLKBUFLR_EN, clkbuflr_en);
	if (bandgap)
		pll_write(base + DSIPHY_PLL_PLL_BANDGAP, bandgap);

	pll_8996->uc = uc;

	return 0;
}

static int dsi_pll_8996_get_provider(struct msm_dsi_pll *pll,
				struct clk **byte_clk_provider,
				struct clk **pixel_clk_provider)
{
	struct dsi_pll_8996 *pll_8996 = to_pll_8996(pll);

	if (byte_clk_provider)
		*byte_clk_provider = pll_8996->provided_clks[DSI_BYTE_PLL_CLK];
	if (pixel_clk_provider)
		*pixel_clk_provider =
				pll_8996->provided_clks[DSI_PIXEL_PLL_CLK];
	return 0;
}

static void dsi_pll_8996_destroy(struct msm_dsi_pll *pll)
{
	struct dsi_pll_8996 *pll_8996 = to_pll_8996(pll);

	msm_dsi_pll_helper_unregister_clks(pll_8996->pdev,
					pll_8996->clks, pll_8996->num_clks);
}

static int pll_8996_register(struct dsi_pll_8996 *pll_8996)
{
	char clk_name[32], parent1[32], vco_name[32];
	struct clk_init_data vco_init = {
		.parent_names = (const char *[]){ "xo" },
		.num_parents = 1,
		.name = vco_name,
		.ops = &clk_ops_dsi_pll_8996_vco,
	};
	struct device *dev = &pll_8996->pdev->dev;
	struct clk **clks = pll_8996->clks;
	struct clk **provided_clks = pll_8996->provided_clks;
	int num = 0;
	int ret;

	DBG("%d", pll_8996->id);

	snprintf(vco_name, 32, "dsi%dvco_clk", pll_8996->id);
	pll_8996->base.clk_hw.init = &vco_init;
	clks[num++] = clk_register(dev, &pll_8996->base.clk_hw);

	snprintf(clk_name, 32, "dsi%dn1_postdiv_clk", pll_8996->id);
	snprintf(parent1, 32, "dsi%dvco_clk", pll_8996->id);

	/* N1 postdiv */
	clks[num++] = clk_register_divider(dev, clk_name,
			parent1, CLK_SET_RATE_PARENT,
			pll_8996->phy_cmn_mmio + DSIPHY_CMN_CLK_CFG0,
			0, 4, CLK_DIVIDER_ONE_BASED, NULL);

	snprintf(clk_name, 32, "dsi%dpllbyte", pll_8996->id);
	snprintf(parent1, 32, "dsi%dn1_postdiv_clk", pll_8996->id);

	/* Byte clock VCO_CLK/N1/8 */
	clks[num++] = provided_clks[DSI_BYTE_PLL_CLK] =
			clk_register_fixed_factor(dev, clk_name,
				parent1, CLK_SET_RATE_PARENT, 1, 8);

	snprintf(clk_name, 32, "dsi%dn1_postdivby2_clk", pll_8996->id);
	snprintf(parent1, 32, "dsi%dn1_postdiv_clk", pll_8996->id);

	/*
	 * Skip the mux for now, force DSICLK_SEL to 1, Add a /2 divider
	 * on the way. Don't let it set parent.
	 */
	clks[num++] = clk_register_fixed_factor(dev, clk_name,
				parent1, 0, 1, 2);

	snprintf(clk_name, 32, "dsi%dpll", pll_8996->id);
	snprintf(parent1, 32, "dsi%dn1_postdivby2_clk", pll_8996->id);

	/* DSICLK. Don't let it set parent */
	clks[num++] = provided_clks[DSI_PIXEL_PLL_CLK] =
				clk_register_divider(dev, clk_name,
				parent1, 0,
				pll_8996->phy_cmn_mmio + DSIPHY_CMN_CLK_CFG0,
				4, 4, CLK_DIVIDER_ONE_BASED, NULL);

	pll_8996->num_clks = num;

	pll_8996->clk_data.clk_num = NUM_PROVIDED_CLKS;
	pll_8996->clk_data.clks = provided_clks;

	ret = of_clk_add_provider(dev->of_node,
			of_clk_src_onecell_get, &pll_8996->clk_data);
	if (ret) {
		dev_err(dev, "failed to register clk provider: %d\n", ret);
		return ret;
	}

	return 0;
}

struct msm_dsi_pll *msm_dsi_pll_8996_init(struct platform_device *pdev, int id)
{
	struct dsi_pll_8996 *pll_8996;
	struct msm_dsi_pll *pll;
	int ret;

	if (!pdev)
		return ERR_PTR(-ENODEV);

	pll_8996 = devm_kzalloc(&pdev->dev, sizeof(*pll_8996), GFP_KERNEL);
	if (!pll_8996)
		return ERR_PTR(-ENOMEM);

	DBG("PLL%d", id);

	pll_8996->pdev = pdev;
	pll_8996->id = id;
	pll_list[id] = pll_8996;

	pll_8996->phy_cmn_mmio = msm_ioremap(pdev, "dsi_phy", "DSI_PHY");
	if (IS_ERR_OR_NULL(pll_8996->phy_cmn_mmio)) {
		dev_err(&pdev->dev, "failed to map CMN PHY base\n");
		return ERR_PTR(-ENOMEM);
	}

	pll_8996->mmio = msm_ioremap(pdev, "dsi_pll", "DSI_PLL");
	if (IS_ERR_OR_NULL(pll_8996->mmio)) {
		dev_err(&pdev->dev, "failed to map PLL base\n");
		return ERR_PTR(-ENOMEM);
	}

	pll = &pll_8996->base;
	pll->min_rate = VCO_MIN_RATE;
	pll->max_rate = VCO_MAX_RATE;
	pll->get_provider = dsi_pll_8996_get_provider;
	pll->destroy = dsi_pll_8996_destroy;
	pll->disable_seq = dsi_pll_8996_disable_seq;
	pll->save_state = dsi_pll_8996_save_state;
	pll->restore_state = dsi_pll_8996_restore_state;
	pll->set_usecase = dsi_pll_8996_set_usecase;

	pll_8996->vco_delay = 1;
	pll_8996->ssc_en = false;

	pll->en_seq_cnt = 1;
	pll->enable_seqs[0] = dsi_pll_8996_enable_seq;

	ret = pll_8996_register(pll_8996);
	if (ret) {
		dev_err(&pdev->dev, "failed to register PLL: %d\n", ret);
		return ERR_PTR(ret);
	}

	return pll;
}
