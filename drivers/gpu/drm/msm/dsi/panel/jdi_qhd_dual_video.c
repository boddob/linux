...
static int jdi_qhd_dual_video_panel_on(struct msm_dsi_panel *msm_panel)
{
	...
	u8 mcap[2] = {0xb0, 0}; /* Manufacturer Command Access Protect */
	u8 interface_setting[2] = {0xb3, 0x14};
	u8 bkl_ctrl_4[20] = {
		0xce, 0x7d, 0x40, 0x48, 0x56, 0x67, 0x78, 0x88, 0x98, 0xa7,
		0xb5, 0xc3, 0xd1, 0xde, 0xe9, 0xf2, 0xfa, 0xff, 0x04, 0x00};
	...
	/* write DPI format */
	if (mipi_dsi_dcs_set_pixel_format(dsi, MIPI_DCS_PIXEL_FMT_24BIT<<4) < 0)
		return -ECOMM;
	usleep_range(5000, 10000);
	...
	if (mipi_dsi_dcs_write(dsi, 0x53, &control_brightness,
		sizeof(control_brightness)) < (sizeof(display_brightness) + 1))
		return -ECOMM;
	usleep_range(5000, 10000);
	...
	/* write interface setting to video through mode */
	if (mipi_dsi_generic_write(dsi, interface_setting,
			sizeof(interface_setting)) < sizeof(interface_setting))
		return -ECOMM;
	usleep_range(5000, 10000);

	if (mipi_dsi_generic_write(dsi, bkl_ctrl_4, sizeof(bkl_ctrl_4)) <
...
static int jdi_qhd_dual_video_panel_off(struct msm_dsi_panel *msm_panel)
{
	struct mipi_dsi_device *dsi = to_mipi_dsi_device(msm_panel->base.dev);
	int ret = 0;
...
static struct msm_dsi_panel_ops jdi_qhd_dual_video_panel_ops = {
	.on = jdi_qhd_dual_video_panel_on,
	.off = jdi_qhd_dual_video_panel_off,
};
...
int jdi_qhd_dual_video_panel_init(struct msm_dsi_panel_desc *desc)
{
	...
	desc->ops = &jdi_qhd_dual_video_panel_ops;
