				...
				struct msm_dsi_phy_clk_request *clk_req)
{
	...
	dsi_phy_write(base + REG_DSI_14nm_PHY_LDO_CNTRL, data);

	dsi_phy_write(base + REG_DSI_14nm_PHY_GLBL_TEST_CTRL, 0x1);
	...
	dsi_phy_write(base + REG_DSI_14nm_PHY_PLL_CNTRL, 0x00);

	wmb(); /* make sure everything is written before reset and enable */

	/* reset digital block */
	dsi_phy_write(base + REG_DSI_14nm_PHY_CTRL_1, 0x80);
	wmb(); /* ensure reset is asserted */
	udelay(100);
	dsi_phy_write(base + REG_DSI_14nm_PHY_CTRL_1, 0x00);
...
{
	dsi_phy_write(phy->base + REG_DSI_14nm_PHY_GLBL_TEST_CTRL, 0);
	dsi_phy_write(phy->base + REG_DSI_14nm_PHY_CTRL_0, 0);
