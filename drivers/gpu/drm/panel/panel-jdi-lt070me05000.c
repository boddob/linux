/*
 * Copyright (C) 2014-2015 InforceComputing
...
static int jdi_panel_init(struct jdi_panel *jdi)
{
	...
	ret = mipi_dsi_dcs_write(dsi, write_display_brightness[0],
					write_display_brightness[1], 1);
	if (ret < 0)
		return ret;

	ret = mipi_dsi_dcs_write(dsi, write_control_display[0],
					write_control_display[1], 1);
	if (ret < 0)
		return ret;

	ret = mipi_dsi_dcs_write(dsi, write_cabc[0],
					write_cabc[1], 1);
	if (ret < 0)
		return ret;
